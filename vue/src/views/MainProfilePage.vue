<template>
  <div class="grid_container">
    ...
    <img v-bind:src="profile.imgUrl"/>
      ...
      <section class="calories">
        <h2 class="other">{{ calorieTotal }} current calories/</h2>
        <span> </span>
        <h2 class="class">{{ profile.goalCalories }} goal calories</h2>

        <!-- v-for="meal in meals" v-bind:key = "meal.id">
   -->
        <h2 class="this">Log 3 Consecutive Meals in One Day to Earn a Dumbbell!</h2>
        <img
          class="stars"
 ...
 -->
    ...
    <section class="meals">
      ...
      <section class="snack">
        <h3>Graph</h3>
        <BarChart class="chart" v-if="calorieTotal && monthlyCalorieTotal" :bar-data="[ calorieTotal, monthlyCalorieTotal ]" :goal-data="[ this.profile.goalCalories, monthlyCalorieGoals ]"/>
      </section>
      <section>
...
export default {
  ...
  computed: {
    ...
    weeklyFood() {
      ...
      return this.food.filter(response => {
        const foodConsumedDate = new Date(response.foodConsumed);
        return now.getWeek() == foodConsumedDate.getWeek();
      })
    },
    yearlyFood() {
      const now = new Date();
    ...
    weeklyCalorieTotal() {
      var calories = 0;
      this.weeklyFood.forEach(element => {
        calories += element.calories;
      })
      return calories;
    },
    ...
    monthlyCalorieGoals() {
      var calories = this.profile.goalCalories;
      calories *= 30;
      return calories;
    }
  },
